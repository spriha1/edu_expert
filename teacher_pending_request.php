...
<body>
	<?php
		...
		$sql = $conn->prepare("SELECT firstname, lastname, email, username FROM users where user_reg_status = 0 AND user_type_id = (SELECT u_id FROM type_of_user where user_type = 'Teacher')");
	    $sql->execute();
